...
import './App.css';
import { addTodo, AllTodo, deleteTodo, DoneTodo, isImportant } from './redux/reducers/todo'
          ...
          <ul>
            {
              todos.map((item) => (
                <li style={{ margin: '20px 0', color: item.isImportant ? 'red' : '', textDecoration: item.isDone ? 'line-through' : '' }} key={item.id} >{item.title}
                  ...
                  <button type='button' style={{ margin: '0 40px' }} onClick={() => { dispatch(isImportant(item.id)) }}>IMPORTANT</button>
                  <button type='button' style={{margin:'0 40px'}} onClick={() => {dispatch(DoneTodo(item.id))}}>DONE</button>
                </li>
              ))
            }
          </ul>
          <button type='button' style={{ margin: '0 50px' }} onClick={(item) => { dispatch(AllTodo(item.id)) }}>Delete-All</button>
        </div>
      </div>
    </div>
  );
